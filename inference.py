...
def constructBayesNet(gameState: hunters.GameState):
    ...
    - populate `variables` with the Bayes Net nodes, i.e., the constants defined below.
    - populate `edges` with every edge in the Bayes Net. We will represent each
      edge as a tuple `(from, to)`, where 'from' and 'to' are the defined variables.
    ...
    max_distance = (X_RANGE - 1) + (Y_RANGE - 1)
    max_obs_value = max_distance + MAX_NOISE

    observations = list(range(max_obs_value + 1))
    variableDomainsDict[OBS0] = observations
    variableDomainsDict[OBS1] = observations
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        result of the eliminate procedure would be 1 (you marginalize 
        all of the unconditioned variables), but it is not a 
        valid factor.  So this simplifies using the result of eliminate.

        The sum of the probabilities should sum to one (so that it is a true 
        conditional probability, conditioned on the evidence). The `normalize`
